classdef PECCASMFS < matlab.apps.AppBase
    ...
    properties (Access = public) %全局变量
        file_path % 默认文件夹地址
        clearValue %默认Unselected
        result_path % 默认result文件夹地址
        pauseValue %判断是否暂停
        resultname %新建文件夹名字变量
        runValue %0为可以运行run，1为停止运行run
        data1 %储存result1数据
        data2 %储存result2数据
        data3 %储存result3数据
        name_path %文件前置名
    ...
    methods (Access = private)
        ...
        function init_run(app)
            ...
            app.Button_run.FontColor = [0 0 0]; %黑色
            app.Button_run.Enable='on'; %可用
        end
        %stop按键不可用
        function init_stop(app)
            app.Button_stop.Text = 'stop';
            app.Button_stop.FontColor = [0 0 0];
            app.Button_stop.Visible='off'; %不显示
            app.Button_stop.Enable='off'; %不可用
        end
        %pause按键不可用
        ...
        function init_ran(app)
            ...
            init_pause(app)
            %初始化
            app.Button_initialize.Enable='on';%初始化可用
            app.Button_file_path.Enable='on';%选择程序文件夹可用
            app.Button_quit.Enable='on';%退出按钮可用
            %初始化结果文件夹value
            app.DropDown_resultFile.Enable='on';
            app.DropDown_resultFile.Value=num2str(0);
            app.clearValue = 0; %默认Unselected
            app.pauseValue = 0; %判断是否暂停，0为否，1为暂停
            app.resultname=''; %新建文件夹名字变量
            app.runValue = 0; %变量为0时，点击run开始计算；点击stop按钮，runValue变为1；若为1，则终止计算。
            app.Button_Export.Enable='on';
            app.Button_Clear.Enable='on';
            app.Button_cModel.Enable='on';%齿数模式可用
        ...
        function pre_run(app)
            ...
            %使按钮不可用
            app.Button_initialize.Enable='off';%初始化不可用
            app.Button_file_path.Enable='off';%选择程序文件夹不可用
            app.DropDown_resultFile.Enable='off';%选择结果文件夹不可用
            app.Button_quit.Enable='off';%退出按钮不可用
            app.Button_run.Enable='off';%运行按钮不可用
            app.Button_Export.Enable='off';
            app.Button_Clear.Enable='off';
            app.Gauge.Value = 0; %更新进度条
            app.Button_cModel.Enable='off';%齿数模式不可用
            %输入不可用
        ...
        function [mistake]=analyseApp(app)
            ...
            if ~exist( file_name ,'file') % # 如果文件不存在则报错
                mistake=1;
                  return  
            end
            ...
            t=floor(m/(app.cln-1));
            m=t*(app.cln-1);
            app.B1=BCalculate(h,t,m);%输入(数据组，步进，数据总量)，输出磁通密度模差值和，左磁通密度模差值和
            file_name =  [app.result_path,'\data\', 'Line-', app.name_path, 'normB_L.txt'];
            [h,m] = dataProcessApp(app,file_name);
            t=floor(m/(app.crn-1));
            m=t*(app.crn-1);
            app.B2=BCalculate(h,t,m);%输入(数据组，步进，数据总量)，输出磁通密度模差值和，右磁通密度模差值和
            mistake=0;
        end
...
%         end
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            %启动后自动运行的函数
            %set(app.Comsol,'closerequestfcn','')%禁用app.comsol窗口的关闭按钮
            app.file_path = 'D:\GitHub\paper1'; % 默认文件夹地址
            app.clearValue = 0; %默认Unselected
            %app.result_path = 'D:\GitHub\paper1\result_default'; % 默认result文件夹地址
            app.pauseValue = 0; %判断是否暂停
            app.resultname; %新建文件夹名字变量
            app.runValue = 0; %0为可以运行run，1为停止运行run
            app.B1=0;%左磁通密度模差值和
            app.B2=0;%右磁通密度模差值和
            userpath('clear');
        ...
        % Button pushed function: Button_initialize
        function Button_initializePushed(app, event)
            button=questdlg('确定初始化吗？','初始化','Yes','No','Yes');%内容，标题，选项（2个），默认选项
            if strcmp(button,'Yes')
                ...
                %app.Button_OpenComsol.Enable='on';
                ...
                app.pauseValue = 0; %判断是否暂停，0为否，1为暂停
                app.resultname=''; %新建文件夹名字变量
                app.runValue = 0; %变量为0时，点击run开始计算，并将runValue变为1；若为1，则终止计算。
                app.EditField_finished.Value = 0;
        ...
        function Button_file_pathPushed(app, event)
            while 1
                judge=uigetdir(app.file_path,'请选择程序文件夹'); %fliepath为文件夹路径
...
%                             app.crn = app.cln;
        ...
        function DropDown_resultFileValueChanged(app, event)
            ...
            while 1
                ...
                elseif app.clearValue==2 %默认文件夹
                    %msgbox('当前选择默认文件夹，点击计算将会清空文件夹中的已有文件','注意','error');
                    app.result_path =fullfile( app.file_path, 'result_default' );
                elseif app.clearValue==3 %新建文件夹
        ...
        function Button_stopPushed(app, event)
            app.runValue=1;%将runValue改为1，run程序中，为1即停止运行
        end

        % Value changed function: Button_Sheet1
        ...
        % Value changed function: Button_Sheet2
        ...
        % Value changed function: Button_Sheet3
        ...
        function Button_ExportPushed(app, event)
            while 1
                ...
                %colName=cell2table(TableData,'VariableNames',colName)
                %TableData=[colName,TableData]
                %AddressFile = fullfile(app.result_path, FileName);
                %writecell(c, AddressFile,'Sheet', SheetName);
                %询问文件名
                button=questdlg('文件名如何命名','文件名','默认','自定义','取消','默认');%内容，标题，选项（2个），默认选项
                if strcmp(button,'自定义')%判断'确定开始计算吗？'的选择
        ...
        % Value changed function: Switch_analyse
        ...
        function Button_ClearPushed(app, event)
            button=questdlg('确认清除吗？','清除','确认','取消','确认');%内容，标题，选项（2个），默认选项
            if strcmp(button,'确认')%判断'确定开始计算吗？'的选择
                app.Table.Data=[];
                if app.Button_Sheet1.Value==1
                    app.data1=[];
                elseif app.Button_Sheet2.Value==1
        ...
        % Value changed function: Button_cModel
...
%                 app.crnin = app.EditField_crn_in.Value;
        ...
        % Button pushed function: Button_openFile
        function Button_openFilePushed(app, event)
            winopen(app.file_path);
        end

        % Button pushed function: Button_openResultFile
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            % Create Comsol and hide until all components are created
            ...
            % Create Label_rateOfProgress_symbol
